            ...
            <div className="w-full md:w-[600px] mt-8">
                <div className="md:hidden flex justify-center">
                    <Select value={selectedTab} onValueChange={setSelectedTab}>
                        <SelectTrigger className="w-full bg-gray-100 border rounded-md">
                ...
                <Tabs defaultValue="Tab1" className="w-full hidden md:block">
                    <TabsList className="flex justify-center border rounded-md p-2 bg-gray-100">
                        {Object.keys(produtos).map((tab) => (
                            <TabsTrigger
                                key={tab}
                                value={tab}
                                className="text-sm md:text-base p-2 rounded-md transition-colors duration-200 hover:bg-gray-200 data-[state=active]:bg-gray-300 mx-1 md:mx-2"
                            >
                                {tab}
                            </TabsTrigger>
                        ))}
                ...
                <div className="md:hidden flex justify-center">
                    <div className="w-full">
                        {Object.entries(produtos).map(([tab, conteudo]) => (
                            selectedTab === tab && (
                                <div key={tab} className="p-2 md:p-4">
                                    <h2 className="text-lg md:text-xl font-semibold mb-2">{conteudo.titulo}</h2>
                                    <p className="text-gray-700 mb-4 text-sm md:text-base">{conteudo.descricao}</p>
                                    <ul className="list-disc list-inside text-sm md:text-base">
                                        {conteudo.recursos.map((recurso, index) => (
